...
import { analyzeMonthlyStatus } from "./gemini";
import { logger } from "./logger";
import { sendEmail } from "./mailer";

export async function checkAndSendMonthlyReports() {
    try {
        ...
        if (!config || !config.reportDay) {
            return;
        }

        // Si es el día configurado, buscamos usuarios que quieran el reporte
        if (currentDay === config.reportDay) {
            ...
            console.log(`[CRON] Procesando informes mensuales para ${users.length} usuarios...`);

            for (const user of users) {
                await generateAndSendMonthlyReport(config, user);
            }
        }
    } catch (error) {
        console.error("Error in checkAndSendMonthlyReports:", error);
    }
}

async function generateAndSendMonthlyReport(config: any, user: any) {
    try {
        ...
        const analysis = await analyzeMonthlyStatus(
            analysisData,
            config.reportPrompt || "Analiza mi situación financiera del mes pasado.",
            config.iaModel
        );
        ...
        await sendEmail({
            to: user.email,
            subject: `📊 Reporte Mensual Mone.yo - ${analysisData.periodo}`,
            html: `
                <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #333;">
                    <div style="text-align: center; margin-bottom: 30px;">
                        ...
                        <p style="color: #64748b; margin: 0; font-size: 14px; font-weight: bold;">HOLA ${user.name.toUpperCase()}, AQUÍ TIENES TU REPORTE:</p>
                        <p style="color: #64748b; margin: 0; font-size: 14px; font-weight: bold;">MES: ${analysisData.periodo}</p>
                    </div>
                    ...
                    <div style="text-align: center; border-top: 1px solid #e2e8f0; padding-top: 20px; margin-top: 40px;">
                        <p style="font-size: 12px; color: #94a3b8; margin: 0;">Este reporte ha sido generado especialmente para ti por la IA de Mone.yo.</p>
        ...
        // Registro del envío
        await logger.info(`[REPORTE MENSUAL IA PROGRAMADO] Enviado a: ${user.email}`, {
            destinatario: user.email,
            periodo: analysisData.periodo,
            contenidoAnalisis: analysis
        });

        console.log(`✅ Informe mensual para ${user.name} (${user.email}) enviado con éxito.`);
    } catch (error: any) {
        await logger.error("Error generating scheduled monthly report", error);
    }
}
